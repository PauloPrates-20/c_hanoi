...
void InitGame() {
    ...
    for(ColTypes i = SOURCE; i < COL_COUNT; i++) {
        ...
        game->cols[i]->discs = malloc(sizeof(Disc)*game->disc_count);
    }
    ...
    for(int i = 0; i < game->disc_count; i++) {
        ...
        game->discs[i] = MakeDisc(i, x, y, BLUE, game->cols[SOURCE]);
    }

    for(int i = 0; i < game->disc_count; i++) {
        game->cols[SOURCE]->discs[i] = game->discs[i];
    }

    game->cols[SOURCE]->last_disc = game->discs[0];
...
void RenderGame() {
    ...
    DrawRectangle(0, SCREEN_HEIGHT - BOTTOM_OFFSET, SCREEN_WIDTH, BOTTOM_OFFSET, RED);
